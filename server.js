...
app.prepare()
    .then(() => {
        ...
        server.use('/static/images', express.static(__dirname + '/static/images'));
        ...
        server.post('/register', urlEncodedParser, function(req, response) {
            ...
            bcrypt.genSalt(saltRounds, function (err, salt) {
                bcrypt.hash(req.body.password, salt, null, function (err, hash) {
                    image.mv('static/images/'+image.name, function(err) {
                        if(err) return response.status(500).send(err);
                    })
                    image2.mv('static/images/'+image2.name, function(err) {
                        if(err) return response.status(500).send(err);
                    })
                    let body = req.body;
                    ...
                    let sql = "INSERT INTO idTest(username, password, ID1, ID2) VALUES ('" + body.username + "', '" + hash + "', '" +imageName+"', '"+imageName2+"')";
                    let searchSQL = "SELECT * FROM idTest WHERE username='"+ body.username +"'";
                        ...
                        bcrypt.compare(body.password, res[0].password, function (err, res) {
                            if (res) {
                                let token = jwt.sign({
                                    ...
                                    xsrfToken: crypto.createHash('md5').update(body.username).digest('hex')
                                }, secret, {
                                    expiresIn: 60 * 60 //1h
                                });
                                response.status(200).json({
        ...
        server.post('/users', urlEncodedParser, function(req, response) {
            let getCurrentUser = database.connection.escape(req.body.currentUser); //prevent SQL injection
            let sql = "SELECT * FROM idTest WHERE username = '"+getCurrentUser+"'";
            database.connection.query(sql, function (err, res, fields){
                if (err) throw err;
        ...
        server.use(unless(['/login', '/register', '/_next'], (req, res, next) => {
